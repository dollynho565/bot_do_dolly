	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\nPor favor não seja um ghost filha da puta❤️`
				...
				teks = `i ala mais um macaco saiu @${num.split('@')[0]} Saiu pq nao transa`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					Badmin: '❌ Este comando só pode ser usado quando o bot se torna um administrador seu membro comum de merda! ❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["554291276766@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Dark\n*YOUTUBE:* https://youtube.com/channel/UCYnJKVT14SQM2GCBJunDaFg n*WPP:* wa.me/+553897349613\n*INSTA:*https://instagram.com/bot_do_dolly?igshid=11oqpm0hqrow5 kk\n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
